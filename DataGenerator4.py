...
# Fonction pour générer des données QAQC - CORRIGÉE pour éviter l'erreur datetime
...
# Fonction pour visualiser un modèle de bloc 2D (section XY)
def visualize_block_model_xy(block_model, selected_metal, z_level, block_size_z, origin_z):
    # Calculer l'indice Z pour la section demandée
    z_index = int((z_level - origin_z) / block_size_z)
    ...
    # Extraire les valeurs X, Y et la teneur
    x = df_section['X'].values
    y = df_section['Y'].values
    grade = df_section[f'Teneur_{selected_metal}'].values
    
    # Créer la figure
    fig, ax = plt.subplots(figsize=(8, 6))
    
    # Tracer la section avec une échelle de couleur logarithmique
    scatter = ax.scatter(x, y, c=grade, s=50, cmap='viridis', norm=LogNorm(), alpha=0.8)
...
if data_type == "Composites":
    ...
    if st.button("Générer le modèle de bloc"):
        if selected_metals:
            ...
            # Afficher la section
            section_image = visualize_block_model_xy(block_model, viz_metal, selected_z, block_size_z, origin_z)
